...
def start_game(root):
           ...
           if not self.finished:
               ...
               if distance != 0:
                   ...
                   if self.rect.colliderect(
                           asteroid.rect) and asteroid_y == new_missile_y and asteroid_x == new_missile_x:
   ...
   class Asteroid(pygame.sprite.Sprite):
       ...
       def reset_position(self):
           ...
           x_cartesian = max(-int(coord_plane_width / (2 * scale)),
                             min(int(coord_plane_width / (2 * scale)), x_cartesian))
...
def open_directions():
   directions_window = tk.Toplevel()
   directions_window.title("Directions")

   # Create a label with the directions message
   directions_label = tk.Label(directions_window,
                               text="Instructions:\n\n"
                                    "Enter the coordinates in the format x,y.\n\n"
                                    "Press Enter to launch a missile.\n\n"
                                    "Destroy the asteroid to earn 10 points.\n\n"
                                    "Be careful! Missing the asteroid will cost a life.\n\n"
                                    "Your goal is to save Earth!", font=("Times New Roman", 14))
   directions_label.pack(padx=20, pady=40)
...
def main():
   ...
   button_color = "white"  # Change the button color to white
   text_color = "black"  # Change the text color to black
   font = ("Arial", 16)
   ...
   start_button = tk.Button(root, text="Start Game", command=lambda: start_game(root), bg=button_color, fg=text_color,
                            font=font)
   start_button.place(relx=0.5, rely=0.3, anchor=tk.CENTER)

   directions_button = tk.Button(root, text="Directions", command=open_directions, bg=button_color, fg=text_color,
                                 font=font)
   directions_button.place(relx=0.5, rely=0.5, anchor=tk.CENTER)

   quit_button = tk.Button(root, text="Quit", command=root.destroy, bg=button_color, fg=text_color, font=font)
